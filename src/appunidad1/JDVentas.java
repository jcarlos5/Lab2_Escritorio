...
//                    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        // TODO add your handling code here:
        try {
            objVenta.eliminarVenta(txtCodVenta.getText(), tblProductos);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(rootPane, e.getMessage());
        }
    ...
    public void setVenta(int venta){
        txtCodVenta.setText(String.valueOf(venta));
        try {
            ResultSet rs = objVenta.listarVenta(venta);
            while (rs.next()){
                if(rs.getString("dni").equals("")){
                    txtDocumento.setText(rs.getString("ruc"));
                    llenarDatos(txtDocumento.getText());
                }else{
                    txtDocumento.setText(rs.getString("dni"));
                    llenarDatos(txtDocumento.getText());
                }
                if(rs.getBoolean("tipocomprobante")){
                    rbtBoleta.setSelected(true);
                    rbtBoleta.setEnabled(false);
                    rbtFactura.setEnabled(false);
                }
            }
            ResultSet det = objVenta.listarDetalle(venta);
            while(det.next()){
                agregarProducto(det.getInt("codproducto"), det.getInt("cantidad"), det.getInt("descuento"));
            }
            btnAnadir.setEnabled(false);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(rootPane, e.getMessage());
