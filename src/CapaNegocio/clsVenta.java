...
public class clsVenta {
    ...
    public void registrar(int cod, float total, float subtotal, float igv, boolean tipo, int cliente, boolean pago, boolean contado, JTable tblProductos) throws Exception{
        try {
...
//        }
    ...
    public void eliminarVenta(String cod, JTable tblProductos) throws Exception{
        try {
            objConectar.conectar();
            con=objConectar.getConnection();
            con.setAutoCommit(false);
            ...
            for (int i=0; i<ctd; i++){
                ...
                strSQL = "DELETE FROM detalle WHERE numventa = "+ cod +" and codproducto = " + tblProductos.getValueAt(i, 0).toString() + ";";
                sent.executeUpdate(strSQL);
            }
            
            strSQL = "DELETE FROM venta WHERE numventa = " + cod + ";";
            sent.executeUpdate(strSQL);
    ...
    public ResultSet listarVenta(int numVenta) throws Exception{
        strSQL = "SELECT v.*, c.dni as dni, c.ruc as ruc,c.nombres, cu.vuelto, cu.ingreso, cu.cancelada FROM venta v inner join cliente c on v.codcliente=c.codcliente inner join cuota cu on v.numventa=cu.codventa"
    ...
    public ResultSet listarDetalle(int numVenta) throws Exception{
        strSQL = "SELECT * FROM detalle WHERE numventa = "+ numVenta+";";
        try {
            rs=objConectar.consultarBD(strSQL);
            return rs;
        } catch (Exception e) {
            throw new Exception("Error al listar detalle por numero de venta");
        }
    }
